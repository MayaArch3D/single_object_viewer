 ...
 * Defines global constants used in other classes 
	...
	@author techgique https://github.com/techgique
*/
...
GIScene.Utils = {
	...
	WorkingMaterial : {
		...
		setWireframe : function (object, wireframeMode){ // 'default'|true|false
			...
			if (object.material && !((object instanceof THREE.Sprite)||(object instanceof THREE.PointCloud))) {
...
GIScene.Control.CameraLight = function (camera, light, config){
	...
	this.activate = function(){
		...
		var camTargetWorld = new THREE.Vector3().setFromMatrixPosition(this.camera.target.matrixWorld);
...
GIScene.Control.Pick = function(camera, config){
	...
	var pickedPointMaterial = new THREE.PointCloudMaterial({
	...
	var pickedPoint = new THREE.PointCloud(pickedPointGeom, pickedPointMaterial);
...
// 	
	...
	this.drawPickResult = function(pickResult) {
		...
		if(resultsLayer.root.children.length==1){
			// set second point 
			var pickedPoint = new THREE.PointCloud(pickedPointGeom.clone(), pickedPointMaterial);
			resultsLayer.root.add(pickedPoint);
	...
	this.activate = function(){
		...
		pickedPointMaterial = new THREE.PointCloudMaterial({
												color:this.config.color,
												sizeAttenuation:false,
												size:32,
												map:pickSymbol,
		...
		var parseGeometry = function(geometry, height) {
			...
			switch (type){
				...
				case "Point" :
					...
					object = new THREE.PointCloud(geom3, material);
					break;
...
GIScene.ModelLoader = function () {
	...
	var callback = function(geometry, materials){
		...
		if(this.format == GIScene.Format.Scene){
			...
			//Mesh or PointCloud?
			var objectType = (geometry.faces.length == 0) ? "PointCloud" : "Mesh";		
			...
			if (materials && materials.length != 0 /*no empty array*/) {
				...
				if (materials.length >= 2) {
					...
					materials = new THREE.MeshLambertMaterial({
						...
						materials = new THREE.PointCloudMaterial({color: new THREE.Color(0x0), size:0.2}); //brown 0xD2B48C
					}
				} else if ( objectType == "PointCloud" ){
					// if PointCloud try to use some of the MeshMaterialProperties
					materials[0].size = 0.2;
					materials[0].vertexColors = THREE.VertexColors;
					materials = new THREE.PointCloudMaterial( materials[0] );
				}else {
				...
				//PointClouds
					...
					materials =undefined;//= new THREE.PointCloudMaterial({ vertexColors: THREE.VertexColors , size:0.2}); //0xD2B48C brown
				}
			}
...
// 
			...
			var mesh = new THREE[ objectType ](geometry, materials); //Mesh or PointCloud
...
// 				
	...
	this.onUnload = function(event) {
		...
		for(var i=0,j=this.controls.length; i<j; i++){
		  if(this.controls[i] !== undefined && this.controls[i].isActive)this.controls[i].deactivate();
		};
